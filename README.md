...
- **Button A 快门**：按下 StickS3 内置 Button A 即触发 BLE 快门序列。
- **相机关机保护**：检测到相机主动断开 BLE 后进入保护态，避免 StickS3 在相机关机流程中再次唤醒相机。
- **手动唤醒恢复**：保护态冷却结束后，按 Button A 才会重新唤醒并连接相机。
...
| 按键 | 功能 |
| --- | --- |
| Button A | LiveView 中触发 BLE 快门；在相机保护态下作为手动唤醒 / 重连按键 |

Button A 即 StickS3 内置按键（`M5.BtnA`），固件在 `loop()` 中轮询 `wasPressed()`。
...
这是当前固件的默认行为。收到 `531 / 533` 断连后会进入 `CAMERA_SLEEP_GUARD`，不会自动发送 Wi-Fi ON。冷却结束后需要按 Button A 才会手动唤醒。

### StickS3 重启后是否会自动连接

会。保护态不写入 NVS，StickS3 重启后从 BLE 扫描流程重新开始。

### Button A 快门日志显示 BLE shutter failed，但相机实际拍摄成功
