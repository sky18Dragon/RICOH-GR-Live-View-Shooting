...
Native 测试必须覆盖：GR III/IV/冲突/Unknown 检测、Unknown 副作用拒绝、GR II ManualOnly、GR3/GR4 Security Profile 常量、地址类型归一化、六位 Passkey 完成/重置/超时、Locked/BondInvalid 单相机策略、普通错误不删 Bond、Operation Mode 安全决策、GR III 可选 Channel 凭据、NVS v3→GR4 Legacy 升级和 v4 往返，以及既有 MJPEG/身份/监督器测试。
...
1. 全新烧录首次配对（确认只读发现后重建为 `GR4_LEGACY`，Numeric Comparison UX 不变）。
2. StickS3 重启后自动重连。
3. 相机重启后自动重连。
4. 相机关机时不被误唤醒。
5. WLAN 快速连接缓存。
...
9. 相机端删除 StickS3 配对后进入 BondInvalid，不自动删 Bond或重新配对。
