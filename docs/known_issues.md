...
1. **机型验证范围**：当前已在 RICOH GR IV、GR IV HDF 和 GR III HDF 上完成实机验证。标准 GR III 与 GR IIIx 仍需在当前分支上独立回归。
2. **GR II 当前不可用**：GR II 缺少当前固件依赖的 BLE 与按需 Wi-Fi AP 控制链路。
...
- `src/PROJECT_NOTES.md` 中部分行可能描述旧行为，例如旧 shutter handle 说明；以后以当前源码和 README 为准。
...
- 标准 GR III 与 GR IIIx 的完整回归。
- GR III HDF 的 Button A 快门发送日志。
- 额外按钮/外设的硬件可行性。
