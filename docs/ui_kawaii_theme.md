...
Kawaii 的 Boot 和 Status 页面使用预渲染 JPEG 背景，以保持参考设计中人物的比例、渐变和纹理；其他页面的背景和角色仍由 LovyanGFX 基础图元绘制：

- `KawaiiTheme.h` 定义柔和薰衣草紫背景、浅色面板、粉色高光以及成功/警告/错误等 RGB565 颜色。
- `KawaiiAssets.h` 保存背景圆形图案的位置/半径及少量固定标签，不保存像素图。
- `assets/boot_background.jpg` 是 240 × 135 的 Boot 背景，只包含紫色纹理和右侧人物，不包含文字、进度或页脚。
- `assets/status_background.jpg` 是 240 × 135 的 Status 背景，只包含紫色纹理和右侧人物，不包含文字、状态灯或按钮。
- `KawaiiBootBackground.h` 是 Boot JPEG 的固件内嵌数据。
- `KawaiiStatusBackground.h` 是上述 JPEG 的固件内嵌数据，由 Status 页面进入时解码一次。
...
Boot 的标题、进度和页脚，以及 Status 的状态灯、电量和按钮，仍根据代码或 `UiModel` 动态绘制在图片之上。两张 JPEG 只用于普通页面，不进入 LiveView Overlay，因此不会增加每帧取景叠加层的工作量。
...
| `KawaiiBootBackground.h` | 240 × 135 Boot JPEG 的内嵌固件数据 |
| `KawaiiStatusBackground.h` | 240 × 135 Status JPEG 的内嵌固件数据 |
| `assets/boot_background.jpg` | 无文字、无进度的 Boot 背景原图 |
| `assets/status_background.jpg` | 无文字、无状态的 Status 背景原图 |
...
| --- | --- | --- |
| Boot | `renderBoot()` | 预渲染紫色背景和右侧单角色、动态标题、固定进度条、页脚、Wi-Fi/电量 Badge |
| Status | `renderStatus()` | `RICOH GR` 顶栏、左侧五行图标灯面板、右侧大角色和左下纵向操作提示 |
| LiveView | `renderLiveViewOverlay()` | 顶部 Live/快门状态、FPS、RICOH GR、Wi-Fi、电量和中间对焦框；不绘制人物或底部操作胶囊 |
...
Boot 页的 `67%`、`FW 1.0.1`，Settings 页的 `1/125s`、`Standard`、`+/-0.0`、`Reset All`、`2 min`，以及部分页脚标签都是代码中的固定视觉样稿，不是实时相机属性或设置状态。Settings 的 Wi-Fi Tile 只根据 `model.wifiConnected` 显示 `On` / `Off`，仍不是可操作开关。
...
| Boot 角色 | 已预渲染进 240 × 135 背景，位于屏幕右侧 |
| Status 角色 | 已预渲染进 240 × 135 背景，约占 `x=115..239`、`y=17..134` |
| LiveView 底部提示 | 不绘制 |
