...
Kawaii 的 Status 页面使用一张预渲染 JPEG 背景，以保持参考设计中人物的比例、渐变和纹理；其他页面的背景和角色仍由 LovyanGFX 基础图元绘制：
...
Status 的状态灯、电量和按钮仍根据 `UiModel` 动态绘制在图片之上，不会把连接状态固化进背景。JPEG 只在普通 Status 页面绘制，不进入 LiveView Overlay，因此不会增加每帧取景叠加层的工作量。
