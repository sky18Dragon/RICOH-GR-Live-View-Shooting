...
| --- | --- | --- |
| Boot | `renderBoot()` | 紫色图案背景、外框、双角色、标题、固定进度条、页脚、Wi-Fi/电量 Badge |
| Status | `renderStatus()` | Camera Connect 卡片、角色、BLE/Wi-Fi/Camera/Battery/Signal 状态行和操作提示 |
| LiveView | `renderLiveViewOverlay()` | 边缘 Live/快门状态、FPS、Wi-Fi、电量、可选型号/帧统计、对焦框、小角色和底部操作条 |
...
Boot 页的 `68%`、`FW 1.0.1`，Settings 页的 `1/125s`、`Standard`、`+/-0.0`、`Reset All`、`2 min`，以及部分页脚标签都是代码中的固定视觉样稿，不是实时相机属性或设置状态。Settings 的 Wi-Fi Tile 只根据 `model.wifiConnected` 显示 `On` / `Off`，仍不是可操作开关。
...
| `UI_FEATURE_WIFI_RSSI` | 开 | Boot、Settings、LiveView 的 Wi-Fi Badge 和 Status 的 Signal 行 |
| `UI_FEATURE_BATTERY` | 开 | Boot、Settings、LiveView 的电量 Badge 和 Status 的 Battery 行 |
| `UI_FEATURE_CAMERA_MODEL` | 开 | LiveView 相机型号标签和 Status 的 Camera 行 |
| `UI_FEATURE_FOCUS_BRACKET` | 开 | LiveView 对焦框 |
| `UI_FEATURE_MASCOTS` | 开 | 页面主角色和 LiveView 小角色；不移除独立的爱心/爪印装饰 |
