...
| **Button B** | Any State (Single Press) | Toggles the display between landscape rotations `1` and `3` (180° flip); the selection persists across restarts |
| **Button B** | Any State (Double Press) | Toggles horizontal mirroring for the camera preview only; UI text and overlays remain unchanged, and the selection persists across restarts |
